...
const viewEmployees=()=>{
    db.query("SELECT employee.id, employee.first_name, employee.last_name, employee.manager_id, role.title, role.salary, department.name AS department FROM employee JOIN role ON employee.role_id = role.id JOIN department ON department.id = role.department_id", (err,results)=>{
        if (err) throw err;
        console.table(results);
        restart();
    })
...
const viewTotalBudget = ()=>{
    ...
    const totalBudgetPrompt = (deptChoices) =>{
        ...
        .then((answers)=>{
            db.query('SELECT SUM(salary) AS total_budget FROM employee JOIN role ON employee.role_id = role.id WHERE role.department_id = ?;', answers.deptId, (err,results)=>{
                if (err) throw err;
                console.table(results);
                restart();
    })
